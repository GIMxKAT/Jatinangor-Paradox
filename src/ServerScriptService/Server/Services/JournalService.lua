...
local log = Log.new("JournalService")
...
function JournalService.Init(registry: { [string]: any })
...
function JournalService.Start()
    Net.OnServerEvent(RemoteNames.Journal_Collect, function(player, fragmentId)
        JournalService.HandleCollect(player, fragmentId)
    end, 5)
end

function JournalService.HandleCollect(player: Player, fragmentId: unknown)
